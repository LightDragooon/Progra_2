    ...
    ui(new Ui::VentanaPrincipal)
{
    ...
    */
    ...
    QMovie* treegif = new QMovie(":/recursos/videos/arbolVida.gif");
    if(treegif->isValid()){
        ui->arbolGif->setMovie(treegif);
        treegif->setSpeed(75);
        treegif->start();
...
void VentanaPrincipal::on_actionPecados_Filtro_triggered()
{

}

void VentanaPrincipal::on_action_rbol_Mundo_triggered()
{
    //Despliega a aquellos que se encuentren en el arbol de la vida
    ArbolPersonasAVL* arbolMundo = new ArbolPersonasAVL();
    arbolMundo->insertarMundo(listaPersonasPlaneta);

    VentanaArbolPersonasAVL ventPersAVL (this);
    ventPersAVL.setArbolAVL(arbolMundo);
    ventPersAVL.exec();
}
